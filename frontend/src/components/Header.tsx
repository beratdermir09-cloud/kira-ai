...
import { ChevronDown, Download, X, Sliders, Menu, GitCompare } from 'lucide-react'
...
export default function Header({
  ...
  const Btn = ({ onClick, children, title: t, hoverColor = '#a78bfa', active = false }: {
    onClick: () => void; children: React.ReactNode; title?: string; hoverColor?: string; active?: boolean
  }) => (
    <button onClick={onClick} title={t} className="p-1.5 rounded-xl transition-all"
      style={{
  ...
  const SelectBox = ({ value, onChange, label }: { value: string; onChange: (v: string) => void; label?: string }) => (
    <div className="relative flex items-center gap-1.5">
      {label && <span className="text-[10px] font-mono" style={{ color: '#4a3a6a' }}>{label}</span>}
      <select value={value} onChange={e => onChange(e.target.value)}
        className="appearance-none text-xs px-3 py-1.5 pr-7 rounded-xl outline-none cursor-pointer transition-all"
        style={{ background: 'rgba(139,92,246,0.04)', border: '1px solid rgba(139,92,246,0.12)', color: '#94a3b8' }}>
        {models.map(m => <option key={m.id} value={m.id} style={{ background: '#0a0814' }}>{m.name}</option>)}
      </select>
      <ChevronDown size={10} className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: '#4a3a6a' }} />
    </div>
  )

  const Tab = ({ id, label }: { id: typeof settingsTab; label: string }) => (
    <button onClick={() => setSettingsTab(id)} className="flex-1 py-1.5 text-[11px] font-medium rounded-lg transition-all"
      style={{
        background: settingsTab === id ? 'rgba(124,58,237,0.12)' : 'transparent',
        color: settingsTab === id ? '#a78bfa' : '#4a3a6a',
        border: settingsTab === id ? '1px solid rgba(139,92,246,0.3)' : '1px solid transparent',
      }}>
      {label}
    </button>
  )
    ...
    <div className="relative">
      <div className="flex items-center justify-between px-4 py-2.5 relative"
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          <button onClick={onMenuClick} className="md:hidden p-1.5 rounded-xl transition-all"
            style={{ background: 'rgba(139,92,246,0.04)', border: '1px solid rgba(139,92,246,0.1)', color: '#4a3a6a' }}>
            <Menu size={15} />
          </button>
          ...
          {!isGuest && personality && personality !== 'default' && (
            <button onClick={() => { setShowSettings(true); setSettingsTab('general') }}
              ...
              style={{ background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(139,92,246,0.2)', color: '#a78bfa' }}
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 flex-wrap justify-end">
          {compareMode ? (
            <>
              ...
              <span className="text-[10px] hidden sm:block" style={{ color: '#2d1f4a' }}>vs</span>
      ...
      {/* Settings panel */}
      {showSettings && (
        <div className="absolute right-4 top-14 z-50 rounded-2xl p-5 w-80 shadow-2xl"
          style={{
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowSettings(false)} style={{ color: '#4a3a6a' }}
          ...
          <div className="flex gap-1 mb-4 p-1 rounded-xl"
            style={{ background: 'rgba(124,58,237,0.04)', border: '1px solid rgba(139,92,246,0.08)' }}>
          ...
          {settingsTab === 'general' && (
            <>
              <div className="mb-5">
                ...
                <div className="flex justify-between text-[10px] mt-2" style={{ color: '#2d1f4a' }}>
              ...
              {!isGuest && onPersonalityChange && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {PERSONALITIES.map(p => (
                      <button key={p.id} onClick={() => onPersonalityChange(p.id)}
                        className="flex flex-col items-start px-3 py-2.5 rounded-xl text-left transition-all"
                        style={{
                          background: personality === p.id ? 'rgba(124,58,237,0.12)' : 'rgba(124,58,237,0.03)',
                          border: `1px solid ${personality === p.id ? 'rgba(139,92,246,0.45)' : 'rgba(139,92,246,0.1)'}`,
                        }}
                        onMouseEnter={e => { if (personality !== p.id) e.currentTarget.style.borderColor = 'rgba(139,92,246,0.3)' }}
                        onMouseLeave={e => { if (personality !== p.id) e.currentTarget.style.borderColor = 'rgba(139,92,246,0.1)' }}>
                        <span className="text-base mb-0.5">{p.emoji}</span>
                        <span className="text-xs font-semibold" style={{ color: personality === p.id ? '#a78bfa' : '#94a3b8' }}>
                          {p.label}
                        </span>
                        <span className="text-[10px] leading-tight mt-0.5" style={{ color: '#2d1f4a' }}>
                          {p.description}
                        </span>
                      </button>
                    ))}
          ...
          {settingsTab === 'compare' && (
            <div>
              <p className="text-xs mb-4" style={{ color: '#4a3a6a' }}>
                Aynı soruyu iki farklı modele aynı anda sor, yanıtları yan yana gör.
              </p>
              <div className="space-y-3 mb-5">
                <div>
                  <label className="text-[10px] font-mono mb-1.5 block" style={{ color: '#4a3a6a' }}>MODEL A</label>
                  <div className="relative">
                    <select value={compareModelA || selectedModel} onChange={e => onCompareModelAChange?.(e.target.value)}
                      className="w-full appearance-none text-xs px-3 py-2 pr-7 rounded-xl outline-none cursor-pointer"
                      style={{ background: 'rgba(139,92,246,0.04)', border: '1px solid rgba(139,92,246,0.15)', color: '#94a3b8' }}>
                      {models.map(m => <option key={m.id} value={m.id} style={{ background: '#0a0814' }}>{m.name}</option>)}
                    </select>
                    <ChevronDown size={10} className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: '#4a3a6a' }} />
                  </div>
                </div>
                <div>
                  <label className="text-[10px] font-mono mb-1.5 block" style={{ color: '#4a3a6a' }}>MODEL B</label>
                  <div className="relative">
                    <select value={compareModelB || selectedModel} onChange={e => onCompareModelBChange?.(e.target.value)}
                      className="w-full appearance-none text-xs px-3 py-2 pr-7 rounded-xl outline-none cursor-pointer"
                      style={{ background: 'rgba(139,92,246,0.04)', border: '1px solid rgba(139,92,246,0.15)', color: '#94a3b8' }}>
                      {models.map(m => <option key={m.id} value={m.id} style={{ background: '#0a0814' }}>{m.name}</option>)}
                    </select>
              ...
              <button onClick={() => { onToggleCompare?.(); setShowSettings(false) }}
                className="w-full py-2.5 rounded-xl text-xs font-semibold transition-all"
                style={{
                  background: compareMode ? 'rgba(124,58,237,0.15)' : 'linear-gradient(135deg, #7c3aed, #0891b2)',
